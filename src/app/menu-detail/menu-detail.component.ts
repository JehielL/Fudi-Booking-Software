...
export class MenuDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.params.subscribe(params => {
      ...
      const ratingsUrl = 'https://217.160.163.48:8080/menus/filter-by-menu/' + id;
      const userUrl = 'https://217.160.163.48:8080/user/' + id;
      
      

      const url = 'https://217.160.163.48:8080/menus/' + id;
      ...
      this.httpClient.get<Dish[]>('https://217.160.163.48:8080/dishes/filter-by-menu/' + id)
  ...
  checkCanEdit(id: number): void {
    this.httpClient.get<boolean>(`https://217.160.163.48:8080/menus/can-edit/${id}`)
    .subscribe(canEdit => {
  ...
  delete(menu: Menu) {
    const url = 'https://217.160.163.48:8080/menus/' + menu.id;
  ...
  save() {
    ...
    this.httpClient.post<Rating>('https://217.160.163.48:8080/ratings', rating).subscribe(rating => {
      this.ratingForm.reset();
      this.loadRatings();
    });

    this.httpClient.get<Rating[]>('https://217.160.163.48:8080/menus/filter-by-menu/' + this.menu?.id)
  ...
  deleteRating(rating: Rating) {
    this.httpClient.delete('https://217.160.163.48:8080/ratings/' + rating.id)
    .subscribe({
      next: response => {
        this.loadRatings();
