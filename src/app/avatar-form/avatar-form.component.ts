...
export class AvatarFormComponent implements OnInit {
  ...
  save() {
    ...
    if (this.isUpdate) {
      this.httpClient.put<User>('https://api.fudi.es/users/account/avatar/' + this.user?.id, formData)
        .subscribe(user => this.navigateToList());
    } else {
      this.httpClient.post<User>('https://api.fudi.es/users/account/avatar/', formData)
