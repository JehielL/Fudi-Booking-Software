...
export class AvatarFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.params.subscribe(params => {
      ...
      this.httpClient.get<User>('https://dev.bitesoftware.es:8080/users/account' + id).subscribe(user => {
        this.userForm.reset({
  ...
  save() {
    ...
    if (this.isUpdate) {
      this.httpClient.put<User>('https://dev.bitesoftware.es:8080/users/account/avatar/' + this.user?.id, formData)
        .subscribe(user => this.navigateToList());
    } else {
      this.httpClient.post<User>('https://dev.bitesoftware.es:8080/users/account/avatar/', formData)
