...
export class RestaurantFormComponent implements OnInit {
  ...
  loadRestaurant(id: string): void {
       ...
       this.httpClient.get<Restaurant>(`https://api.fudi.es/restaurant/${id}`).subscribe(restaurant => {
  ...
  save() {
    ...
    if (id && this.isUpdate) {
      timer(500).pipe(
        switchMap( () => this.httpClient.put<Restaurant>(`https://api.fudi.es/restaurant/${id}`, formData)))
            .subscribe(response => {
      ...
      timer(500).pipe(
        switchMap( () => this.httpClient.post<Restaurant>('https://api.fudi.es/restaurant', formData)))
            .subscribe(response => {
                this.router.navigate(['/restaurant', response.id, 'detail']);
                this.showSpinner = false;
            });
