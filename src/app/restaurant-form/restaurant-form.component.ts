...
export class RestaurantFormComponent implements OnInit {
  ...
  loadRestaurant(id: string): void {
       ...
       this.httpClient.get<Restaurant>(`https://biteapp.store:8080/restaurant/${id}`).subscribe(restaurant => {
  ...
  save() {
    ...
    if (id && this.isUpdate) {
      timer(500).pipe(
        switchMap( () => this.httpClient.put<Restaurant>(`https://biteapp.store:8080/restaurant/${id}`, formData)))
            .subscribe(response => {
      ...
      timer(500).pipe(
        switchMap( () => this.httpClient.post<Restaurant>('https://biteapp.store:8080/restaurant', formData)))
            .subscribe(response => {
                this.router.navigate(['/restaurant', response.id, 'detail']);
                this.showSpinner = false;
            });
