  ...
  ngOnInit(): void {
    this.activatedRoute.params.subscribe(params => {
      ...
      timer(500).pipe(
        ...
        timer(500).pipe(
          switchMap(() => this.httpClient.get<boolean>('https://dev.bitesoftware.es:8080/restaurants/can-edit/' + id)
        )).subscribe(canEdit => {
          this.canEdit = canEdit;
          this.showSpinner = false;
        });

        const menusUrl = `https://dev.bitesoftware.es:8080/menus/byRestaurant/${id}`;
        timer(500).pipe(
      ...
      this.httpClient.get<Booking[]>('https://dev.bitesoftware.es:8080/bookings/filter-by-restaurant/' + id)
